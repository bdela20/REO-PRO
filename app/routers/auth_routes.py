...
def google_login():
    ...
    print("="*50)
    print(f"Client ID from config: {current_app.config['GOOGLE_CLIENT_ID']}")
    print(f"Has Client Secret: {'Yes' if current_app.config['GOOGLE_CLIENT_SECRET'] else 'No'}")
    print(f"Redirect URI from config: {current_app.config['GOOGLE_REDIRECT_URI']}")
    ...
    # THE KEY CHECK - Does generated URI match what's in Google Console?
    print(f"\n🔍 URI COMPARISON:")
    print(f"What Flask generates: {redirect_uri}")
    print(f"What Google expects: http://localhost:5001/auth/google/callback")
    print(f"Do they match? {'✅ YES' if redirect_uri == 'http://localhost:5001/auth/google/callback' else '❌ NO - THIS IS THE PROBLEM!'}")
    print("="*50 + "\n")
...
def google_callback():
    ...
    try:
        ...
        print(f"App config CLIENT_ID: {current_app.config.get('GOOGLE_CLIENT_ID', 'NOT SET')}")
